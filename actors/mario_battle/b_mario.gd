...
const JUMP_RETURN_HEIGHT : int = 80

var jumping : bool = false # Playing the first jump animation
var allow_jumping_back : bool = false # Allow playing the return jump animation (OK, EXCELLENT)

var allow_jump_bounce : bool = false # Allow bouncing on an enemy
var allow_fail_bounce : bool = true # Allow bouncing off of an enemy
...
const JUMP_FAIL_HEIGHT : int = 30
# ^ the second bounce is this value / 2.5
...
#	print(jump_rating) # General print for testing
...
func jump_action(_delta): 
	if start_jump_attack == true:
		match jump_stage:
			...
			3: # Play the jump attack's bounce animation
				if allow_jump_bounce == true:
...
func jump_action_cancelled(_delta): # Handles the sine wave for a cancelled jump (Fail, OK, GOOD)
	match jump_rating:
		"Fail":
			...
			if allow_fail_bounce == true:
				if jump_fail_bounce_amt == 1: # First bounce for failed jump
					...
					jump_attack_fail_pos.y - position.y)  * jump_fail_progression) - 5)

					position.x = position.x + (
					(jump_attack_fail_pos.x - position.x) * jump_fail_progression)

					jump_fail_progression += 2.5 * _delta
					int(jump_fail_progression)
				...
				if jump_fail_bounce_amt == 2: # Second bounce for failed jump
					...
					position.y = position.y - (
					((JUMP_FAIL_HEIGHT / 2.5) * sin(jump_fail_progression * PI) - (
					jump_attack_fail_pos.y - position.y)  * jump_fail_progression) - 5)

					position.x = position.x - 3 * jump_fail_progression

					jump_fail_progression += 5.0 * _delta
					int(jump_fail_progression)

					if jump_fail_progression >= 1:
						jump_fail_progression = 0
						allow_fail_bounce = false
						anime_state = "jump_fail_land"
						# -> _on_Puppet_animation_finished()
						jump_rating = ""
		...
		"OK":
			...
			if allow_jumping_back == true and jump_progression < 1.0:
				anime_state = "jump"

				position.y = jump_attack_end_pos.y - (
					JUMP_RETURN_HEIGHT * sin(jump_progression * PI) - (
					origin_point.y - jump_attack_end_pos.y)  * jump_progression)

				position.x = jump_attack_end_pos.x + (
					(origin_point.x - jump_attack_end_pos.x) * jump_progression)

				jump_progression += 2 * _delta
				int(jump_progression)

			if allow_jumping_back == true and jump_progression >= 0.5:
				anime_state = "jump_fall"

			if allow_jumping_back == true and jump_progression >= 1.0:
				anime_state = "jump_land"
				jump_progression = 0
				allow_jumping_back = false
				jump_rating = ""

	# Handles running back from a failed jump
	if running_back_jump == true:
		jumping = false
		anime_state = "run_left"
...
func hammer_action(_delta):
	if start_hammer_attack == true:
		match hammer_stage:
			...
			3: # Running toward the enemy (tween)
...
func end_turn():
	...
	jump_rated = false
	jump_fail_bounce_amt = 1
	allow_fail_bounce = true
	running_back_jump = false
...
func _on_AnimationPlayer_animation_finished(anim_name):
	match anim_name:
		"jump_check":
			jumpcheck.show_rating()
			allow_jump_bounce = false
			allow_jumping_back = true
